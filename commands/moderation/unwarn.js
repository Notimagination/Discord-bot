...
module.exports = {
name: "unwarn",
description: "Remove warnings from members",
usage: "unwarn [mention | ID]",
example: "unwarn @Someone\nunwarn 12345667891011",
perm: ["Kick members"],
aliases: [""],
category: "Moderation",
...
let zero = await warns.get(`Server ID: ${message.guild.id}.Warns`)
...
embed.setDescription("<:disapproved:803627127089397793> | You cannot unwarn a member with a rank equal to or greater than yours.")
embed.setColor(0xff0000)
return message.channel.send(embed).then(m => m.delete({ timeout: 5000 }))
...
warns.subtract(`Server ID: ${message.guild.id}.Warns`, 1)

let warnings = await warns.get(`Server ID: ${message.guild.id}.Warns`)

embed.setThumbnail(member.user.displayAvatarURL({dynamic: true}))
embed.setDescription(`<:approved:803627105493319720> | **Member unwarned**\n\n**Member:**\n${member}\n**ID:**\n${member.id}\n**Moderator:**\n${message.author}\n**Total warns:**\n${warnings}`)
